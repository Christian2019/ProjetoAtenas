...
func updateScroll(scroll):
	...
	#0-hades 1-poseidon 2-zeus
	scroll.get_node("Big/AnimatedSprite2D2").frame=2
	...
	scroll.get_node("Big/Labels/Active/value").text="zeus will create rain of lightning bolts for 15 seconds causing "+str(warrior_dash_poseidon_chance[quality])+" (SD) to enemies every 0.2s that explode"
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="eeduce the cooldown by every electrified* enemy for "+str(warrior_dash_poseidon_destructionInstinct*100)+"Limit 0.1s Cooldown."
	scroll.get_node("Big/Labels/ExtraInfo/value").text="* electrified: receive extra damage from all sources for 5 seconds"
