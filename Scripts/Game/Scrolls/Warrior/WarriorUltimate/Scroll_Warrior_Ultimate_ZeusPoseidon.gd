...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="reduce the cooldown by every electrified* enemy for "+ str(warrior_ultimate_divine_zeus_cdReduction) +"s. limit 0.1s cooldown."+" create a kraken for 5 seconds with "+str(warrior_ultimate_divine_poseidon_tentaclesQuantity)+" tentacles. each tentacle copys turret skill."
