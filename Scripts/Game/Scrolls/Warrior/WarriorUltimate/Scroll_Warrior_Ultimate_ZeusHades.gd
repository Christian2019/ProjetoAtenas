...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="create a cerberus* for the rest of the wave."+" reduce the cooldown by "+ str(warrior_ultimate_divine_zeus_cdReduction) +"s."
	#scroll.get_node("Big/Labels/ExtraInfo").visible=false
	scroll.get_node("Big/Labels/ExtraInfo/value").text="cerberus*: creature that deals " + str(warrior_ultimate_divine_hades_cerberusDamage) +" sd plus the amount of hp gain by your skeletons."+" electrified*: receive extra x damage from all sources for 5 seconds."
