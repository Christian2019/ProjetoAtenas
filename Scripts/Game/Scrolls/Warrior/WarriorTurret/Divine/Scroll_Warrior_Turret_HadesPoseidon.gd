...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/Active/value").text="Turrets attacks causes a lightning bolt "+str(warrior_dash_poseidon_chance[quality])+" (SD) to strike nearby foes"
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="Turrets attacks generate a chain-lightning with "+str(warrior_dash_poseidon_destructionInstinct*100)+" extra bounces"
