...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/Active/value").text="turret attacks creates a ray wich pierces and slows all enemies. it accumulates damage that explodes after 5 seconds"+str(warrior_dash_poseidon_chance[quality])+" (SD)"
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="increase explosion area by "+str(warrior_dash_poseidon_destructionInstinct*100)+"%"
