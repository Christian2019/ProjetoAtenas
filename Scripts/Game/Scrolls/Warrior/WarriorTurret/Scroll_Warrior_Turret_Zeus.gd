...
var quantity
var warrior_turret_zeus_damage
var warrior_turret_zeus_extraBounces

func start():
	sellPrice=Global.ScrollController.getCurrentScrollPrice(self)
	passive=AllSkillsValues.warrior_turret_zeus_passive
	#passive2=AllSkillsValues.warrior_dash_hadesPoseidon_passive2
	
	quantity=AllSkillsValues.turretsQuantity
	warrior_turret_zeus_damage=AllSkillsValues.warrior_turret_zeus_damage
	warrior_turret_zeus_extraBounces=AllSkillsValues.warrior_turret_zeus_extraBounces
...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/Active/value").text="turret’s attack causes a lightning bolt that deal "+ str(warrior_turret_zeus_damage[quality]) +" sd to strike nearby foes."
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="turret’s attack became chain-lightning produce "+ str(warrior_turret_zeus_extraBounces) +" extra bounce"
