...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/Active/value").text="your attack emits chain-lightning that deals  "+str(warrior_dash_poseidon_chance[quality])+" when you damage an enemy. produces <valor> extra bounce "
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="enemies hit by normal attack gets electrified* "+str(warrior_dash_poseidon_destructionInstinct*100)+"%."
	scroll.get_node("Big/Labels/ExtraInfo/value").text="*electrification: enemies receive extra damage from all sources for 5 seconds."
