...
func updateScroll(scroll):
	...
	scroll.get_node("Big/Labels/Active/value").text="gain temporary attack speed for 5 seconds after hitting an enemy "+str(warrior_dash_poseidon_chance[quality])+"%"
	scroll.get_node("Big/Labels/LegendaryDivineBonus/value").text="gain frenezy* "+str(warrior_dash_poseidon_destructionInstinct*100)+"%."
